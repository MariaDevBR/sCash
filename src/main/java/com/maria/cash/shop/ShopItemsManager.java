...
public class ShopItemsManager {
	...
	public ShopItems getShopItems(ItemStack icon) {
		return shopItemsMap.entrySet().stream().map(Map.Entry::getValue).filter(it -> it.getIcon().isSimilar(icon)).findFirst().orElse(null);
	}
	...
	private void setupShopItems() {
		for (String path : shopItemsFile.getConfigurationSection("Itens").getKeys(false)) {
			...
			if (category != null) {
				ShopItems shopItems = new ShopItems(icon, category, broadcastActionBar, price, slot, commands, broadcastMessage, broadcast, chat, actionBar);
