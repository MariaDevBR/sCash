...
public class CashMethods {
    ...
    public boolean hasCash(Player player, double amount) {
        ...
        if (!cashAPI.hasCash(player, amount)) {
            main.sendMessage(player, messages.getCashInsufficient().replace("{necessario}", Format.formatNumber(amount)).replace("{cash}", Format.formatNumber(cash)));
    ...
    public void viewCashOthers(CommandSender player, Player target, double cash) {
        ...
        othersBalance.forEach(msg -> player.sendMessage(msg.replace("{cash}", Format.formatNumber(cash)).replace("{player}", target.getName())));
    ...
    public void sendCash(Player player, Player target, double amount) {
        ...
        sendCash.forEach(msg -> player.sendMessage(msg.replace("{quantia}", Format.formatNumber(amount)).replace("{player}", target.getName())));
        receivedCash.forEach(msg -> target.sendMessage(msg.replace("{quantia}", Format.formatNumber(amount)).replace("{player}", player.getName())));
    ...
    public void voucherCash(Player player, double amount, int quantity) {
        ...
        voucher.forEach(msg -> player.sendMessage(msg.replace("{cash}", Format.formatNumber(amount)).replace("{quantia}", Format.formatNumber(quantity))));
    ...
    public void giveVoucherCash(CommandSender sender, Player target, double amount, int quantity) {
        ...
        main.sendMessage(sender, voucher.replace("{cash}", Format.formatNumber(amount)).replace("{quantia}", Format.formatNumber(quantity)).replace("{player}", target.getName()));
